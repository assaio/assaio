...
| `runtime` | v0.24 | **Experimental.** `runtime inspect` snapshots a self-hosted vLLM server's and NVIDIA DCGM exporter's own metrics endpoints — `--vllm-url`/`--vllm-file` and `--dcgm-url`/`--dcgm-file`, each pair mutually exclusive — and reports availability, the metric families found, current gauges with their units and where the unit came from, and **what is unavailable, which is never rendered as zero**. Read-only: no time series is stored, no self-hosted cost is estimated, no GPU or runtime change is recommended, and nothing is written to the data directory. A counter is never presented as a rate and a histogram reports its observation count only — one snapshot cannot produce either, and every printed counter says so. Bounds are flags: `--timeout`, `--max-bytes`, `--max-redirects` (`0` forbids redirects). A hosted vendor's accelerators stay `unknown`: no local signal reveals them. Sits behind an explicit demand gate — see [ROADMAP.md](ROADMAP.md) — and may be removed. |
