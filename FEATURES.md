...
| `digest` | v0.17 | Markdown summary of what **moved** since the last run — totals, model and project movers, verdict changes — for cron/launchd; delivery stays the user's script. Each run stores its own basis (bounded to 12, no session content). Declares a weak comparison rather than reporting through it: overlapping windows, differing window lengths, or a parser change between runs that corrected history. |
