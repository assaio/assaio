...
| Exec **metric** plugin, any language | v0.1 | `<command> analyze`, Input JSON on stdin → handshake + one Result on stdout; `metrics:` in config (ADR 0004). The Input gained six fields in v0.17 (`skills`, `agents`, `turnSizing`, `cacheMisses`, `delegation`, `planMonthlyCost`) and two in v0.21 (`trace`, `historyStart`), so a plugin reaches what the built-ins reach. **Breaking in v0.22:** a result must declare its `layer` (ADR 0013) or it is rejected whole. |
