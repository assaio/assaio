...
| `dashboard` | v0.1 | Writes the self-contained offline Assay HTML report; pseudonymized by default, `--no-anonymize` opt-out. A team panel appears only on a central store, and since v0.14 a member's row shows engagement only — lines and spend are the team's total, never a per-person figure. |
