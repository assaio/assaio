...
Outputs leave the process only when asked:
...
## The local Evidence Graph path

`assaio-agent evidence` is a second read path rather than another ingest path. It opens the
