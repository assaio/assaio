...
First, it requested classic Prometheus text (`Accept: text/plain`). In that format, vLLM's Python client names each counter family `<name>_total`, but assaio looked it up by its base name. All six counters it read — prompt and generation tokens, prefix-cache hits and queries, finished requests, and preemptions — were therefore missing on both versions measured and in any exposition written in `prometheus_client`'s classic text format. Saving the endpoint with the guide's own `curl` produced the same format.

Second, it read the KV-cache gauge, two prefix-cache counters, and time-per-output-token under names vLLM hid in v0.11.0 (2025-10-02) or v0.12.0 (2025-12-03), before the reader shipped in v0.24.0. Against vLLM v0.12.0's metric names, it read 4 of 12 capabilities and reported 8 unavailable; against v0.10.2's, it read 6 and reported 6. No other versions were measured. The guide's advice to check for an exporter flag could restore only hidden names: the old cache names on vLLM 0.11.x, and time-per-output-token on releases that still carried it hidden (v0.12.0 to at least v0.14.0; gone by v0.16.0). No flag restored the counters.

The constructed test fixture used OpenMetrics and the old names, so no test caught either defect. The result was measured on 2026-09-25 with expositions generated by `prometheus_client` using vLLM v0.12.0 and v0.10.2 metric names and types, read by the v0.29.0 binary. No live server was tested.

The DCGM reader was unaffected. assaio has no telemetry, so whether anyone relied on these readings is unknown. The command is removed in v0.30.0, so it no longer makes the claim.
