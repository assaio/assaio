...
Shipped in v0.22 and enforced rather than promised: `internal/layer` is the vocabulary,
`analyze.Validator` requires a `Layer()` method — a compile error, not a forgettable field — the
exec metric protocol rejects a result without one, and a test fails if a built-in ever declares
`outcome` or `impact`. Of the twenty-one built-in reads, eighteen are activity and three are
output; none claims an outcome. See [ADR 0013](docs/adr/0013-measurement-layers.md).
...
| **Worth opening twice** ✓ | A first run produces something worth acting on this week, and a reason to come back the next one | shipped, findings are ordered by what is worth a week's attention rather than rendered as a wall of equals, with the ordering's reasons shown and nothing promoted when everything is weak (`B148`); a digest fit for cron says what moved without anyone opening a terminal, and declares when the comparison itself cannot be trusted (`B11`); the label that makes every metric comparable per kind of work is derived through a rule engine instead of typed (`B152`); and the metric-plugin envelope carries every field a built-in validator reads, so an outside author can reproduce what the core publishes (`B155`); and an assay can be posted in public with redaction as the feature rather than a flag (`B149`) — `share` renders a reel, a still poster and the post text beside them from figures `analyze` already published, and no field it renders can hold a repository, member, path, branch, skill or sub-agent name. One half of `B149` was dropped rather than deferred: repositories appear as a count and never as a sorted spend distribution, because the ordering and proportions are what a pseudonym gets read back out of. The trend the item also asked for is `B184` |
| **Everything the logs already say** | Without a server, a repository or a credential, `assaio` reads every signal the local logs carry, proves the reading is right, and turns it into more conclusions than any vendor dashboard draws from the same file | every source's unread fields are inventoried and either extracted or documented as deliberately skipped, done in v0.10 ([the audit](docs/extending/source-fields.md)); what it found is extracted (`B107`–`B114`, of which the cache tier and miss cause shipped in v0.12 as `B108`); the activity gap closes where a log carries the data (`B39`, `B72`); the local-view metrics land as one file each with their caveats (`B28`–`B37`, `B02`, `B78`, `B79`); and a parser proves itself against evidence it did not generate, so a mis-read fails a test instead of shipping, shipped in v0.14 (`B137`); and the session stops being only a total — a content-free timeline of what the agent actually did, read for the patterns that make a session expensive, so "why did this cost so much" has an answer (`B147` — the Claude Code reading landed across v0.20.0 and v0.21.0; the Codex reading remains, see *What is being worked on now*) — which the milestone above is what keeps readable, since every detector this adds is one more finding competing for the same week's attention |
...
Twenty-one reads render as twenty-one reads. A reader facing a wall of equally weighted verdicts acts
...
surfaces are a projection of the live registries, the published pages declare which of their
claims are checkable, and the check runs in both directions: a claim with nothing behind it fails,
and so does a shipped capability nothing published. Prose stays outside it deliberately — a guard
that implied it read the prose would be exactly the false green this project refuses. Before that:
the extension surface stopped being weaker than the core it extends — six prepared-input fields
never crossed the metric-plugin boundary while five shipped validators read them, so an
out-of-tree author could not reproduce a third of what assaio publishes, and a canary now fails
the build when a new one does not cross (`B155`); a digest reports what *moved* and declares when
the comparison itself is weak, including the failure a single-window surface cannot have — a
parser that changed between the runs (`B11`); and a label is derived through a rule engine over
configurable sources rather than typed, yielding nothing where a repository has no convention
(`B152`). Before those: the
...
skipped with the reason ([the audit](docs/extending/source-fields.md)).
