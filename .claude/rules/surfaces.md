...
- **Every `.md` under `docs/` is published or excused** in `internal/docs/guides.go`
  ...
  are skipped as directories. `docs/reference.json`, `site/reference.html`, `site/docs.html` and
  `site/docs/*` are generated: `make docs`, never edited.
...
- **A new parser changes `PRIVACY.md`, `README.md`'s source table, `site/llms.txt`, `AGENTS.md`'s
  layout and `CITATION.cff`'s abstract** in the same commit.
