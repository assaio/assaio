...
func parseMetricResult(doc []byte, pluginName string) (analyze.Result, []string, error) {
	...
	// Three fields are answers about the window rather than about this metric, so a plugin's
	...
	// them. Withheld is what assaio kept out of the envelope it sent, which assaio is the
	// authority on. All three became decodable the moment they were added to Result; clearing
	// them here is what keeps the contract the same after as before.
