...
type Dashboard struct {
	...
	// them, and what a window with none of them says instead.
	WorthAttentionLabel   string
	OfSuffix              string
	RankingRule           string
	NothingWorthAttention string
...
var enDashboard = Dashboard{
	...
	WorthAttentionLabel:   "Worth a week's attention",
	OfSuffix:              "of",
	RankingRule:           "Order is these reasons, not a score; findings whose reasons match are ordered by name. Every read is below.",
	NothingWorthAttention: "Nothing this window. Every read is either fine, withheld, or resting on too little to act on — so nothing is promoted rather than leading with the least weak one.",
