// Package pseudonym derives the short, stable labels every shareable surface uses in place
// of a name a person chose -- a project, a member, a skill. It owns the per-install secret
// those labels are keyed by, which is why it is a package of its own rather than a helper
// inside whichever surface happened to need it first.
package pseudonym
...
// hexLen is the number of hex characters kept from the HMAC, e.g.
...
// keyFilename is the per-install secret's name under paths.DataDir().
const keyFilename = "pseudonym.key"

// keySize is the persisted secret's length in bytes: 256 bits, sized for the
// HMAC-SHA256 it keys, not for the (much shorter) truncated output.
const keySize = 32

// For derives a short, stable label for name under kind (e.g. "project", "member"),
// so a shareable export can carry an identity without exposing the real value, e.g.
// For("member", "alice") -> "member-a1b2c3d4e5". Deterministic per install: the
...
func For(kind, name string) string {
	if name == "" {
		return "unknown"
	}
	return withSecret(kind, name, installSecret())
}

// withSecret is For's pure HMAC step, parameterized on the secret so it is
...
func withSecret(kind, name string, secret []byte) string {
	h := hmac.New(sha256.New, secret)
	_, _ = h.Write([]byte(kind + ":" + name))
	return kind + "-" + hex.EncodeToString(h.Sum(nil))[:hexLen]
}
...
// for this process only, so For degrades to per-run rather than per-install
...
// loadOrCreateSecret reads the keySize-byte secret at path, generating and
...
// bytes, which is what makes For's output stable across separate CLI invocations.
...
// randomSecret generates a fresh keySize-byte secret from a CSPRNG -- never
