...
//
// It also moves when the payload's shape does not: once a build reads the same data into
// different verdicts, a stored `good` and a fresh `neutral` for the same metric are the binary
