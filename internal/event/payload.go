...
// vcs.commit, scm.pull_request, scm.review, ci.check, delivery.merge|revert|survival and
// ai.session -- and each lands here with the collector that fills it: one constant, one
// payload struct. Declaring a name is a commitment; declaring a struct nothing produces is
...
var types = []string{TypeUsage, TypeEdit}
